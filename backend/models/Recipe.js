...
const Recipe = {
  ...
  // CONSULTA COMPLEJA ANALÍTICA: Calcula el total de macronutrientes de la receta cruzando 3 tablas
  async getRecipeMacros(recipeId) {
    const sql = `
      SELECT 
        r.id AS recipe_id,
        r.name AS recipe_name,
        ROUND(SUM((i.calories_per_100g / 100) * ri.amount), 1) AS total_calories,
        ROUND(SUM((i.protein_per_100g / 100) * ri.amount), 1) AS total_protein,
        ROUND(SUM((i.carbs_per_100g / 100) * ri.amount), 1) AS total_carbs,
        ROUND(SUM((i.fat_per_100g / 100) * ri.amount), 1) AS total_fat
      FROM recipes r
    ...
    return rows[0] || { message: "Esta receta aún no tiene ingredientes registrados para calcular macros." };
  }
};
